...
export class SyncWorker {
  ...
  public startSync(sessionId?: string): void {
    this.isRunning = true;
  ...
  public async drainQueue(sessionId?: string): Promise<void> {
    this.isRunning = true;
  ...
  private async processQueue(targetSessionId?: string): Promise<void> {
    ...
    try {
      ...
      for (const chunk of unsynced) {
        ...
        const task: Promise<void> = this.uploadChunkWithRetry(chunk)
          .then(() => {})
          .finally(() => {
  ...
  public async uploadChunkWithRetry(chunk: LocalVideoChunk): Promise<boolean> {
    const chunkKey = `${chunk.sessionId}:${chunk.sequenceNumber}`;
  ...
  public async uploadChunk(chunk: LocalVideoChunk): Promise<boolean> {
    if (!chunk.blob) {
