...
chrome.runtime.onMessageExternal?.addListener((message, sender, sendResponse) => {
    ...
    const isAllowedOrigin = senderUrl.includes('openplan.ai') ||
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
    ...
    if (tab.active && changeInfo.status === 'complete') {
        ensureActiveSessionKnown()
            ...
            .catch((err) => logger.warn(`[ServiceWorker] Widget injection on tab update failed for tab ${tabId}:`, err));
    }
});
...
chrome.tabs.onActivated.addListener(({ tabId }) => {
    ensureActiveSessionKnown()
        ...
        .catch((err) => logger.warn(`[ServiceWorker] Widget injection on tab activation failed for tab ${tabId}:`, err));
});
...
async function ensureActiveSessionKnown() {
    if (!activeSessionId) {
...
async function handleServiceWorkerMessage(message, sender) {
    ...
    switch (message.action) {
        case 'START_RECORDING': {
            const payload = message.payload || { title: 'Meeting Recording' };
            ...
            try {
                ...
                if (!initResponse.ok) {
                    ...
                    logger.error(`Backend session init rejected (HTTP ${initResponse.status}): ${bodyText}. Chunk sync for ${sessionId} will fail until retried.`);
                }
            }
            catch (err) {
...
async function forwardControlActionToOffscreen(message, meta) {
    if (!activeSessionId) {
        return {
            success: false,
            error: {
...
async function injectRecordingWidget(tabId) {
    try {
        ...
        if (!tab.url || !/^https?:\/\//.test(tab.url)) {
            ...
            logger.warn(`[ServiceWorker] Skipping recording widget injection: tab ${tabId} is not an injectable http(s) page (url=${tab.url ?? 'unknown'})`);
            return;
        }
