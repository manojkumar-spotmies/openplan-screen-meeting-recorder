...
describe('RecordingWidget (recording-widget.tsx)', () => {
    ...
    function findCall(action) {
        const calls = sendMessageMock.mock.calls;
        return calls.find((call) => call[0].action === action);
    }
    function queueControlResponse(response) {
        sendMessageMock.mockImplementationOnce((_message, callback) => {
    ...
    beforeEach(() => {
        ...
        sendMessageMock = vi.fn((message, callback) => {
            ...
            Promise.resolve().then(() => callback({ success: true, data: {}, meta: { timestamp: '', requestId: '' } }));
        });
        globalThis.chrome = {
            runtime: {
    ...
    it('sends STOP_RECORDING via the existing stop/finalization flow when Stop Recording is clicked', async () => {
        ...
        const stopButton = buttons.find((b) => b.textContent?.includes('Stop Recording'));
        await act(async () => {
    ...
    it('toggles the microphone off without sending STOP_RECORDING', async () => {
        ...
        const buttons = Array.from(shadow.querySelectorAll('button'));
        const micButton = buttons.find((b) => b.textContent?.includes('Microphone'));
        await act(async () => {
    ...
    it('toggles system audio off without affecting the microphone or stopping the recording', async () => {
        ...
        const buttons = Array.from(shadow.querySelectorAll('button'));
        const sysAudioButton = buttons.find((b) => b.textContent?.includes('System Audio'));
        await act(async () => {
